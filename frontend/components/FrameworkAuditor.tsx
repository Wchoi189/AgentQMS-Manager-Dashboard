...
import { ShieldCheck, AlertTriangle, CheckCircle, Loader2, ArrowRight, Terminal, Wand2, FileText, LayoutTemplate, Sparkles } from 'lucide-react';
...
const AI_TEMPLATES = [
  { type: 'Generic', label: 'Organization Efficiency', description: 'Assess structure, consistency, and maintainability.' },
  { type: 'File Reorganization', label: 'File Reorganization', description: 'Suggest file relocations and renaming.' },
  { type: 'Folder Reorganization', label: 'Folder Redesign', description: 'Analyze folder structure and architecture.' },
  { type: 'General Audit', label: 'General Audit', description: 'Identify deprecated and outdated content.' },
  { type: 'Coding Standards', label: 'Coding Standards', description: 'Check adherence to design principles.' },
  { type: 'Documentation Quality', label: 'Doc Quality', description: 'Verify standardization and lifecycle.' },
  { type: 'Automation Usage', label: 'Automation Check', description: 'Identify automation opportunities.' },
];
...
const FrameworkAuditor: React.FC = () => {
  ...
  const [selectedAiTemplate, setSelectedAiTemplate] = useState(AI_TEMPLATES[0]);
  ...
  const handleAiAudit = async () => {
    ...
    try {
      const data = await auditDocumentation(inputContent, selectedAiTemplate.type);
      setResult(data);
    } catch (e) {
      console.error(e);
      setResult({
        ...
        <div>
          ...
          {mode === 'ai' && (
            <p className="text-xs text-slate-500 mt-1 flex items-center gap-1">
              ...
              AI Analysis evaluates documentation organization, consistency, and maintainability patterns.
            </p>
          )}
        </div>
      ...
      {mode === 'ai' ? (
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="mb-4 bg-slate-800/50 p-3 rounded-lg border border-slate-700">
              <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-2">
                <LayoutTemplate size={12} /> Analysis Templates
              </div>
              <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                {AI_TEMPLATES.map(template => (
                  <button
                    ...
                    className={`whitespace-nowrap px-3 py-1.5 rounded text-xs font-medium transition-colors border ${selectedAiTemplate.type === template.type
            ...
            <textarea
              ...
              placeholder={`Paste content for ${selectedAiTemplate.label}...\n\n${selectedAiTemplate.description}`}
              value={inputContent}
            ...
            <button
              ...
              className={`w-full py-3 rounded-lg flex items-center justify-center gap-2 font-semibold transition-all ${isAuditing || !inputContent
                ? 'bg-slate-700 text-slate-400 cursor-not-allowed'
                : 'bg-purple-600 hover:bg-purple-500 text-white shadow-lg shadow-purple-900/20'
                }`}
            >
              {isAuditing ? <Loader2 className="animate-spin" /> : <Sparkles size={18} />}
              {isAuditing ? `Running ${selectedAiTemplate.label}...` : `Run ${selectedAiTemplate.label}`}
            </button>
          </div>
          ...
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-6 overflow-y-auto">
            {!result ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-500 opacity-50">
                ...
                <p>Select a template and paste content for analysis...</p>
              </div>
            ) : (
              <div className="animate-in slide-in-from-bottom-4 duration-500">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h3 className="text-lg font-semibold text-white">{selectedAiTemplate.label} Report</h3>
                    <p className="text-xs text-slate-400">Analysis Type: {selectedAiTemplate.type}</p>
                  </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col animate-in fade-in">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 h-full">
            {/* Artifacts List */}
            <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 flex flex-col min-h-0">
              <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4 flex items-center gap-2 flex-shrink-0">
                ...
                <div className="flex-1 overflow-y-auto space-y-2 custom-scrollbar min-h-0">
                  {artifacts.map(artifact => (
                    <button
                      ...
                      className={`w-full text-left p-2 rounded-lg border transition-all ${selectedArtifact?.id === artifact.id
            ...
            <div className="md:col-span-2 bg-slate-800 rounded-xl border border-slate-700 p-6 flex flex-col">
              ...
              <div className="mt-auto space-y-4">
                ...
                <button
                  ...
                  disabled={runningTool !== null}
                  className={`w-full py-3 rounded-lg flex items-center justify-center gap-2 font-semibold transition-all ${runningTool !== null
                    ? 'bg-slate-700 text-slate-400 cursor-not-allowed'
